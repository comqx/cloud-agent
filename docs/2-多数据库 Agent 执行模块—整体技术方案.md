...
目标：为已完成的 tiangong-deploy 通信层，设计并实现云下 Agent 的多数据库执行模块（Golang），支持 MySQL、PostgreSQL、MongoDB、Elasticsearch、Doris、ClickHouse 等主流后端执行器。模块要具备统一任务接收、执行、结果上报、限流/并发控制、审计与可观测性。
...
云上控制台/API
    │  (任务下发 via existing tiangong-deploy WebSocket channel)
    ▼
...
预留功能（后续迭代）：
	•	Secret Manager：Agent 不直接存储明文凭据。tiangong-deploy 下发任务时仅传 secret_ref，Agent 从本地安全存储（或 KMS）取明文连接信息。
...
预留功能（后续迭代）：
	...
	•	追踪：支持 OpenTelemetry tracing，从 tiangong-deploy 下发到 Agent 执行链路完整追踪。
