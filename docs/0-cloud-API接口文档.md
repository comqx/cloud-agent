# Cloud Agent API 接口文档

本文档描述了 Cloud Agent 服务提供的核心 API 接口，供其他服务调用。
...
Cloud Agent 支持多种数据库的执行操作，包括 MySQL、PostgreSQL、MongoDB、Elasticsearch、ClickHouse 和 Doris。所有数据库执行接口使用统一的接口格式，通过 `type` 字段区分不同的数据库类型。
